...
void	str_counters_create(void)
{
	...
	{
		g_counters[i] = string_get_elem(string_create("1", 1.0f - MENU_WIDTH + d_x * (i + 1),
				SPLITER_Y_3 - (SPLITER_Y_3 - SPLITER_Y_4 - SPLITER_HEIGHT) / 1.5,
				STRING_COUNTERS_FONTSIZE, vec2(1.0f, 1.0f), g_window));
		create_line_vao(4 + i, g_counters[i]->translate[12], g_counters[i]->translate[13]
